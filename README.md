...
本机器人**通过 Webhook 回调接收消息**：在 QQ 开放平台「开发 → 回调配置」勾选
`GROUP_MESSAGE_CREATE` 等事件后，平台会把**群聊全量消息** POST 到本机的 Webhook 服务
（`bot/core/webhook.py`，端口 9091），从而支持**免 @ 的自然语言触发**；同时机器人
通过官方 WebSocket 长连接保持在线与心跳（平台要求）。因此回调地址必须公网可达，
需配合下方内网穿透使用。
